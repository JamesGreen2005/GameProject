...
var projectile_scene = preload("res://attack.tscn")
var can_shoot = true
...
func handle_movement(delta):
	...
	if Input.is_action_just_pressed("player1up") and jumps < max_jumps:
		velocity.y = -5000
		jumps += 1
	...
	if Input.is_action_just_pressed("player1slam") and not is_on_floor():
		velocity.y = 4000
		
func handle_shooting():
	if Input.is_action_pressed("player1attack") and can_shoot:
